...
{
	if(!force) Gamepad[idx].ff.g_pEffect[effidx]->Stop();
	...
	if(  Gamepad[idx].ff.g_dwNumForceFeedbackAxis == 1 ) {
		...
		rglDirection[0] = 0;
		Gamepad[idx].ff.pf.lOffset = Gamepad[idx].ff.xForce;
		...
		Gamepad[idx].ff.pf.lOffset = magnitude;
		Gamepad[idx].ff.pf.dwPeriod = period;
	}
...
{
	if(!Gamepad[idx].ff.g_pEffect[effidx]) {
		DIEFFECT eff;
		DIPERIODIC pf = { 0 };
