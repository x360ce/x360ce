...
namespace x360ce.App
{
	public class Helper
	{
		...
		const int IMAGE_NT_OPTIONAL_HDR32_MAGIC = 0x10b; // PE
		const int IMAGE_NT_OPTIONAL_HDR64_MAGIC = 0x20b; // PE+
		...
		// https://github.com/samskivert/ikvm-monotouch/blob/master/reflect/Reader/ModuleReader.cs
		public static ProcessorArchitecture GetProcessorArchitecture(string filePath)
		{
			...
			ushort magic = 0;
			if (br.ReadUInt16() == 0x5A4D) //check the MZ signature
			{
			...
			bool COR_IS_32BIT_REQUIRED = (((_flags) & (COMIMAGE_FLAGS_32BITREQUIRED | COMIMAGE_FLAGS_32BITPREFERRED)) == (COMIMAGE_FLAGS_32BITREQUIRED));
			bool COR_IS_32BIT_PREFERRED = (((_flags) & (COMIMAGE_FLAGS_32BITREQUIRED | COMIMAGE_FLAGS_32BITPREFERRED)) == (COMIMAGE_FLAGS_32BITREQUIRED | COMIMAGE_FLAGS_32BITPREFERRED));
			switch (machineType)
			{
				// Any CPU: PE32,  32BIT: 0
				// x86:     PE32,  32BIT: 1
				// x64:     PE32+, 32BIT: 0

				case IMAGE_FILE_MACHINE_I386:
					// Check to see if the CLR header contains the 32BITONLY flag, if not then the image is actually AnyCpu.
					if (magic == IMAGE_NT_OPTIONAL_HDR32_MAGIC) return ProcessorArchitecture.X86; // 32-bit
					else return ProcessorArchitecture.MSIL; // AnyCPU
				case IMAGE_FILE_MACHINE_AMD64: return ProcessorArchitecture.Amd64; // 64-bit
				case IMAGE_FILE_MACHINE_IA64: return ProcessorArchitecture.IA64; // 64-bit
