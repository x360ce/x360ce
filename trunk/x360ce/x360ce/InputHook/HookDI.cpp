    ...
    if(pInst && pInst->dwSize == sizeof(DIDEVICEINSTANCEA))
    {
        for(DWORD i = 0; i < iHookThis->GetHookCount(); i++)
        {
            ...
            if(padconf.GetHookState() && IsEqualGUID(padconf.GetProductGUID(),pInst->guidProduct))
            {
                ...
                std::string strTrueguidProduct = Misc::GUIDtoStringA(pInst->guidProduct);
                std::string strHookguidProduct = Misc::GUIDtoStringA(HookInst.guidProduct);

                PrintLog(LOG_HOOKDI,"%s","GUID change:");
                PrintLog(LOG_HOOKDI,"from %s",strTrueguidProduct.c_str());
                PrintLog(LOG_HOOKDI,"to   %s",strHookguidProduct.c_str());
                ...
                if(iHookThis->CheckHook(iHook::HOOK_NAME))
                {
                    ...
                    PrintLog(LOG_HOOKDI,"from \"%s\"",OldProductName.c_str());
                    PrintLog(LOG_HOOKDI,"to   \"%s\"",HookInst.tszProductName);

                    PrintLog(LOG_HOOKDI,"%s","Instance Name change:");
                    PrintLog(LOG_HOOKDI,"from \"%s\"",OldInstanceName.c_str());
                    PrintLog(LOG_HOOKDI,"to   \"%s\"",HookInst.tszInstanceName);
                }
    ...
    if(pInst && pInst->dwSize == sizeof(DIDEVICEINSTANCEW))
    {
        ...
        for(DWORD i = 0; i < iHookThis->GetHookCount(); i++)
        {
            ...
            if(padconf.GetHookState() && IsEqualGUID(padconf.GetProductGUID(),pInst->guidProduct))
            {
                ...
                strTrueguidProduct = Misc::GUIDtoStringW(pInst->guidProduct);
                strHookguidProduct = Misc::GUIDtoStringW(HookInst.guidProduct);

                PrintLog(LOG_HOOKDI,"%s","GUID change:");
                PrintLog(LOG_HOOKDI,"from %ls",strTrueguidProduct.c_str());
                PrintLog(LOG_HOOKDI,"to   %ls",strHookguidProduct.c_str());
                ...
                if(iHookThis->CheckHook(iHook::HOOK_NAME))
                {
                    ...
                    PrintLog(LOG_HOOKDI,"%s","Product Name change:");
                    PrintLog(LOG_HOOKDI,"from \"%ls\"",OldProductName.c_str());
                    PrintLog(LOG_HOOKDI,"to   \"%ls\"",HookInst.tszProductName);

                    PrintLog(LOG_HOOKDI,"%s","Instance Name change:");
                    PrintLog(LOG_HOOKDI,"from \"%ls\"",OldInstanceName.c_str());
                    PrintLog(LOG_HOOKDI,"to   \"%ls\"",HookInst.tszInstanceName);
                }
    ...
    if(pdidi)
    {
        ...
        for(DWORD i = 0; i < iHookThis->GetHookCount(); i++)
        {
            ...
            if(padconf.GetHookState() && IsEqualGUID(padconf.GetProductGUID(), pdidi->guidProduct))
            {
                ...
                strTrueguidProduct = Misc::GUIDtoStringA(pdidi->guidProduct);

                GUID guidProduct = { iHookThis->GetFakePIDVID(), 0x0000, 0x0000, {0x00, 0x00, 0x50, 0x49, 0x44, 0x56, 0x49, 0x44} };
                pdidi->guidProduct = guidProduct;

                strHookguidProduct = Misc::GUIDtoStringA(pdidi->guidProduct);
                ...
                if(iHookThis->CheckHook(iHook::HOOK_NAME))
                {
                    ...
                    PrintLog(LOG_HOOKDI,"%s","Product Name change:");
                    PrintLog(LOG_HOOKDI,"from \"%s\"",OldProductName.c_str());
                    PrintLog(LOG_HOOKDI,"to   \"%s\"",pdidi->tszProductName);

                    PrintLog(LOG_HOOKDI,"%s","Instance Name change:");
                    PrintLog(LOG_HOOKDI,"from \"%s\"",OldInstanceName.c_str());
                    PrintLog(LOG_HOOKDI,"to   \"%s\"",pdidi->tszInstanceName);
                }
    ...
    if(pdidi)
    {
        ...
        for(DWORD i = 0; i < iHookThis->GetHookCount(); i++)
        {
            ...
            if(padconf.GetHookState() && IsEqualGUID(padconf.GetProductGUID(), pdidi->guidProduct))
            {
                ...
                strTrueguidProduct = Misc::GUIDtoStringW(pdidi->guidProduct);

                GUID guidProduct = { iHookThis->GetFakePIDVID(), 0x0000, 0x0000, {0x00, 0x00, 0x50, 0x49, 0x44, 0x56, 0x49, 0x44} };
                pdidi->guidProduct = guidProduct;

                strHookguidProduct =  Misc::GUIDtoStringW(pdidi->guidProduct);
                ...
                if(iHookThis->CheckHook(iHook::HOOK_NAME))
                {
                    ...
                    PrintLog(LOG_HOOKDI,"%s","Product Name change:");
                    PrintLog(LOG_HOOKDI,"from \"%ls\"",OldProductName.c_str());
                    PrintLog(LOG_HOOKDI,"to   \"%ls\"",pdidi->tszProductName);

                    PrintLog(LOG_HOOKDI,"%s","Instance Name change:");
                    PrintLog(LOG_HOOKDI,"from \"%ls\"",OldInstanceName.c_str());
                    PrintLog(LOG_HOOKDI,"to   \"%ls\"",pdidi->tszInstanceName);
                }
...
HRESULT STDMETHODCALLTYPE HookGetPropertyA (LPDIRECTINPUTDEVICE8A This, REFGUID rguidProp, LPDIPROPHEADER pdiph)
{
    ...
    if(&rguidProp == &DIPROP_VIDPID)
    {
        ...
        PrintLog(LOG_HOOKDI,"from %08X to %08X",dwTruePIDVID);
        PrintLog(LOG_HOOKDI,"to   %08X",reinterpret_cast<LPDIPROPDWORD>(pdiph)->dwData);
    }
    ...
    if(iHookThis->CheckHook(iHook::HOOK_NAME))
    {
        if (&rguidProp == &DIPROP_PRODUCTNAME)
        {
            ...
            PrintLog(LOG_HOOKDI,"%s","Product Name change:");
            PrintLog(LOG_HOOKDI,"from \"%ls\"",TrueName);
            PrintLog(LOG_HOOKDI,"to   \"%ls\"",reinterpret_cast<LPDIPROPSTRING>(pdiph)->wsz);
        }
    }
