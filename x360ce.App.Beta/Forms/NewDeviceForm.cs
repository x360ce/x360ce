...
namespace x360ce.App.Controls
{
    public partial class NewDeviceForm : Form
    {
        ...
        void ws_LoadSettingCompleted(object sender, SoapHttpClientEventArgs e)
        {
            ...
            {
                MainWindow.Current.MainPanel._bwm.SetBodyInfo("Setting was not found.");
            }
            else
            {
                ...
                SettingsManager.Current.ResumeEvents();
                MainWindow.Current.MainPanel._bwm.SetBodyInfo("Settings loaded into '{0}.{1}' successfully.", _padIndex + 1, _di.ProductName);
            }
            this.DialogResult = System.Windows.Forms.DialogResult.OK;
        }
        ...
        string GetTitle(string fileName, string fileProductName)
        {
            var list = new List<string>();
            list.Add("Internet");
            if (!string.IsNullOrEmpty(fileName)) list.Add(fileName);
            if (!string.IsNullOrEmpty(fileProductName) && fileName != fileProductName) list.Add(fileProductName);
            if (list.Count == 1) list.Add("Default Settings");
